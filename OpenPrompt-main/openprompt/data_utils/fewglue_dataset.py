...
sys.path.append("/Work21/2023/zhuangning/code/OpenPrompt-main/OpenPrompt-main/openprompt")
import random
...
class RecordProcessor(FewGLUEDataProcessor):
    ...
    @staticmethod
    def get_examples(data_dir, split, seed=42, max_train_candidates_per_question: int = 10) -> List[InputExample]:
        examples = []
        path = os.path.join(data_dir, "{}.jsonl".format(split))
        ...
        with open(path, encoding='utf8') as f:
            for idx, line in enumerate(f):
                ...
                for question_json in questions:
                    ...
                    if len(candidates) > max_train_candidates_per_question - len(answers):
                        entity_shuffler.shuffle(candidates)
                        candidates = candidates[:max_train_candidates_per_question - len(answers)]
                        
                    train_entities = candidates+answers
                    entity_shuffler.shuffle(train_entities)

                    for entity_idx , entity in enumerate(train_entities):
                        ...
                        guid = f'{split}-p{idx}-q{question_idx}-a{entity_idx}'
                        meta = {
                            'passage_idx': idx,
                            'question_idx': question_idx,
                            'entity': entity
