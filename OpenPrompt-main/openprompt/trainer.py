...
class BaseRunner(object):
    ...
    def early_stop(self,score):
        if (self.best_score is not None) and (((self.best_score-score) >= 0) == self.config.checkpoint.higher_better):
            self.no_better_epoch += 1
        else:
            self.no_better_epoch = 0

        logger.info(f"no better epoch : {self.no_better_epoch}")
        
        if self.no_better_epoch >= 10:
            return -1
        return 1
    ...
    def fit(self, ckpt: Optional[str] = None):
        ...
        for self.cur_epoch in range(self.cur_epoch, self.num_epochs):
            ...
            continue_training_better = self.early_stop(score)
            copy = None
            ...
            #
            if continue_training_better == -1:
                logger.info("Stop training by earlystop")
                break
